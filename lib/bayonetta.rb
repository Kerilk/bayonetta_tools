...
module Bayonetta
  ...
  def self.extract_eff(filename, big=false)
    ...
    directory_info.each_with_index { |info, i|
      ...
      puts d_name
      if dir_name == "EST\0" || dir_name == "SAD\0" || dir_name == "SST\0"
        File::open("#{d_name}/data."+dir_name.delete("\0").downcase, "wb") { |f2|
      ...
      file_info = file_number.times.collect {
        f.read(8).unpack("#{l}2")
      }
