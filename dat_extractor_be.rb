...
file_number = f.read(4).unpack("N").first
puts "Found #{file_number} files!"


file_starts_offset = f.read(4).unpack("N").first
file_extensions_offset = f.read(4).unpack("N").first
file_names_offset = f.read(4).unpack("N").first
file_sizes_offset = f.read(4).unpack("N").first

f.seek(file_starts_offset)
file_offsets = f.read(4*file_number).unpack("N*")

f.seek(file_names_offset)
filename_length = f.read(4).unpack("N").first
...
file_sizes = f.read(4*file_number).unpack("N*")
